...
@endif
<h4 class="mt-5 mb-3 text-center">Ubah Data Absensi</h4>

<div class="col-lg-5">
<form method="post" action="{{ route('absensi.update', $data->absen_id) }}">
    @csrf
    <div class="mb-3">
    <label for="absen_id" class="form-label">ID Absen</label>
    <input type="text" class="form-control" id="absen_id" name="absen_id" value="{{ $data->absen_id }}" disabled>
  </div>
  <div class="mb-3">
    <label for="check_in" class="form-label">Jam Masuk</label>
    <input type="time" class="form-control" id="check_in" name="check_in" value="{{ $data->check_in }}">
  </div>
  <div class="mb-3">
    <label for="check_out" class="form-label">Jam Keluar</label>
    <input type="time" class="form-control" id="check_out" name="check_out" value="{{ $data->check_out }}">
  </div>
  <div class="mb-3">
    <label for="employee_id" class="form-label">ID Employee</label>
    <select class="form-control" id="employee_id" name="employee_id" value="{{ $data->employee_id }}">
  ...
  <div class="mb-3">
    ...
    <select name="jadwal_id" id="jadwal_id" value="{{ $data->jadwal_id }}">
